...
-- 인사과에 속한 사원들 중에 휴가를 사용한 사원의 내역 조회 (부서명은 '인사')
select *
...
-- 영업 부서 사원의 사원명, 폰번호, 부서명, 휴가사용 이유, 소속본부 조회
select e.emp_id, e.phone, d.dept_id, v.reason, d.unit_id
from employee e
join vacation v on e.emp_id = v.emp_id
...
/********************************************************************************
  ...
  -------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------
	  실수형	   	  float        4byte        부동소수점             거의 사용 X
				  double       8byte	    디폴트값              통계수치
   -------------------------------------------------------------------------
	  문자형(고정)   char		   고정길이	    빠른공간확보(장)        코드값
									        공간낭비(단)
      문자형(가변)	  varchar	   가변길이	    가장 많이 사용         이름, 주소 등
   -------------------------------------------------------------------------
	  텍스트        text         ~ 64kb       긴 텍스트 저장         게시글
...
-- emp 테이블 생성 
-- emp_id : 4, ename : 5, hire_date : date, salary : 4
create table emp(
...
-- ename 컬럼의 크기를 varchar(10)으로 변경 => 크게 변경은 정상 실행
select * from emp;
...
select * from emp;
-- S001 사번의 폰번호 업뎃, 업데이트 모드 허용으로 수정

set sql_safe_updates = 0;
update emp
	set phone = '010-0208-0930'
    where eid = 'S001';

-- 모든 사원의 폰 번호를 '010 - 2222- 8888'로 수정
update emp
	set phone = '010-2222-8888';
select * from emp;
desc emp;
...
-- emp 테이블에email 컬럼 추가 후 not null 제약 정의
-- 1 ) 컬럼 추가 시 null 허용
...
desc emp;
select * from emp;
update emp set email = 'jinjin@gmail.com';
...
-- 휴가를 많이 사용한 사원순으로 조회, 행번호, 사원명, 휴가 일수 (총합)
